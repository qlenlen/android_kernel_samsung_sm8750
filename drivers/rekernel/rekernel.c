 ...
 */
 ...
 static void (*re_binder_transaction_buffer_release)(
     ...
     bool is_failure);
 static void (*re_binder_alloc_free_buf)(struct binder_alloc *alloc,
 ...
 static inline bool
 rekernel_is_jobctl_frozen_compatible(struct task_struct *task)
 {
 #ifdef KERNEL_5_10
 ...
 {
     return (cgroup_task_frozen(task) ||
         rekernel_is_jobctl_frozen_compatible(task) ||
         rekernel_is_frozen_state_compatible(task->group_leader) ||
 ...
 static int sendMessage(char *packet_buffer, uint16_t len)
 {
     ...
     return netlink_unicast(netlink_socket, socket_buffer, USER_PORT,
                    MSG_DONTWAIT);
 }
 
 #if defined(KERNEL_5_15) || defined(KERNEL_6_1)
 void line_binder_alloc_new_buf_locked(void *data, size_t size,
 ...
 void line_binder_reply(void *data, struct binder_proc *target_proc,
                struct binder_proc *proc, struct binder_thread *thread,
 ...
 #ifdef KERNEL_6_1
 static long line_copy_from_user_nofault(void *dst, const void __user *src,
                     size_t size)
 {
     long ret = -EFAULT;
 ...
 static long line_copy_from_user_compatible(void *dst, const void __user *src,
                        size_t size)
 {
 #ifdef KERNEL_6_1
 ...
 void line_binder_transaction(void *data, struct binder_proc *target_proc,
                  ...
                  struct binder_transaction_data *tr)
 {
     ...
     if ((tr->flags & TF_ONE_WAY) /* async binder */
         ...
         buf_data_size = tr->data_size > INTERFACETOKEN_BUFF_SIZE ?
                     ...
                     tr->data_size;
         if (!line_copy_from_user_compatible(buf_data,
             ...
             if (netlink_socket != NULL) {
                 ...
                 snprintf(
                     binder_kmsg, sizeof(binder_kmsg),
                     "type=Binder,bindertype=transaction,oneway=1,from_pid=%d,from=%d,target_pid=%d,target=%d;",
                     proc->pid, task_uid(proc->tsk).val,
                     target_proc->pid,
                     task_uid(target_proc->tsk).val);
                 sendMessage(binder_kmsg, strlen(binder_kmsg));
 ...
 static inline void __nocfi binder_release_entire_buffer(
     struct binder_proc *proc, struct binder_thread *thread,
 ...
 static int __nocfi binder_proc_transaction_pre(struct kprobe *p,
                            struct pt_regs *regs)
 {
     struct binder_transaction *t =
         (struct binder_transaction *)regs->regs[0];
 ...
 void line_signal(void *data, int sig, struct task_struct *killer,
          struct task_struct *dst)
 {
     if (!dst || !killer)
         return;
 
     if (line_is_frozen(dst) && (sig == SIGKILL || sig == SIGTERM ||
 ...
 int register_binder(void)
 {
     ...
     rc = register_trace_android_vh_binder_alloc_new_buf_locked(
         line_binder_alloc_new_buf_locked, NULL);
     if (rc != LINE_SUCCESS) {
     ...
     rc = register_trace_android_vh_binder_trans(line_binder_transaction,
                             NULL);
     if (rc != LINE_SUCCESS) {
 ...
 static unsigned int rekernel_pkg_ip4_in(void *priv,
                     struct sk_buff *socket_buffer,
 ...
 static unsigned int rekernel_pkg_ip6_in(void *priv,
                     struct sk_buff *socket_buffer,
                     const struct nf_hook_state *state)
 {
     struct sock *sk;
 ...
 rekernel_pkg_ip_out(void *priv, struct sk_buff *socket_buffer,
             const struct nf_hook_state *state)
 {
     return NF_ACCEPT;
 }
 ...
 static void netlink_rcv_msg(struct sk_buff *socket_buffer)
 {
     ...
     if (socket_buffer->len >= nlmsg_total_size(0)) {
         ...
         snprintf(netlink_kmsg, sizeof(netlink_kmsg),
              "Successfully received data packet! %d",
              netlink_count);
         kmsg = netlink_kmsg;
 ...
 int __nocfi register_kp(void)
 {
     ...
     re_binder_transaction_buffer_release = (void *)re_kallsyms_lookup_name(
         "binder_transaction_buffer_release");
     re_binder_alloc_free_buf =
         (void *)re_kallsyms_lookup_name("binder_alloc_free_buf");
     re_binder_stats = (void *)re_kallsyms_lookup_name("binder_stats");
 
     if (re_binder_transaction_buffer_release == NULL ||
         re_binder_alloc_free_buf == NULL || re_binder_stats == NULL) {
         rc = -EINVAL;
 ...
 static int __init start_rekernel(void)
 {
     ...
     for (netlink_unit = NETLINK_REKERNEL_MIN;
         ...
         netlink_socket = (struct sock *)netlink_kernel_create(
             &init_net, netlink_unit, &cfg);
         if (netlink_socket != NULL)
             break;
     }
